...
bool InitScene()
{
	...
	/*
		...
		FillMode -
			确定渲染时要使用的填充模式。可选的有用于线框渲染的D3D11_FILL_WIREFRAME，或者用于实体渲染的D3D11_FILL_SOLID，通常为默认值。

		CullMode -
			确定渲染时要使用的剔除模式。D3D11_CULL_NONE禁用剔除，D3D11_CULL_FRONT用于剔除正面（因此正面不会被渲染），D3D11_CULL_BACK用于剔除背面，这是默认值。

		FrontCounterClockwise -
			确定三角形是正面还是背面。如果此参数为TRUE，则如果三角形的顶点在渲染目标上是逆时针方向，则它将被视为面向前方。如果它们是顺时针方向，则视为面向后方。
			如果此参数为FALSE，则反之亦然。

		DepthBias -
			指定添加到给定像素的深度值。默认值为0.0f。

		DepthBiasClamp -
			指定像素的最大深度偏差。默认值为0.0f。

		SlopeScaledDepthBias -
			指定给定像素的斜率上的标量。默认值为0.0f。

		DepthClipEnable -
			根据与相机的距离启用或禁用剪裁。

		ScissorEnable -
			启用或禁用剪刀矩形剔除。活动剪刀矩形外的所有像素都被剔除。默认值为FALSE。

		MultisampleEnable -
			启用或禁用多重采样抗锯齿。

		AntialiasedLineEnable -
			启用或禁用行抗锯齿。请注意，此选项仅适用于启用Alpha混合，绘制线条以及MultisampleEnable成员为FALSE时。默认值为FALSE。
	*/
