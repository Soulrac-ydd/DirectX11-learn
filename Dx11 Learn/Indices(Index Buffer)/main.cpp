...
bool InitScene()
{
	...
	indexBufferDesc.BindFlags = D3D11_BIND_INDEX_BUFFER; //告诉IA这个缓冲区是索引缓冲区
	...
	d3d11DevCon->IASetIndexBuffer(squareIndexBuffer, DXGI_FORMAT_R32_UINT, 0); //将索引缓冲区绑定到管道的IA阶段。
	...
	vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER; //告诉IA这个缓冲区是顶点缓冲区
	...
	UINT offset = 0;
	d3d11DevCon->IASetVertexBuffers(0, 1, &squareVertBuffer, &stride, &offset); //将顶点缓冲区绑定到管道的IA阶段。
